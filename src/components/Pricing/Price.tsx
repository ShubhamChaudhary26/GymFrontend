    ...
    <section className="py-10 relative text-white">
      <div className="text-center mb-16">
        <h2 className="textHeadingmobile md:textHeadinglaptop">
          Your Fitness Goals, <span className="text-yellow-400">Our Expertise</span>
        </h2>
        ...
        {plans.map((plan: any) => (
          <div
            ...
            className={`relative p-8 rounded-2xl shadow-lg transition-all duration-300 hover:scale-105 ${plan.popular
                ? "bg-gradient-to-b from-yellow-900/30 to-gray-900 border border-yellow-500/30 ring-2 ring-yellow-400"
          ...
          >
            ...
            <div className="text-center mb-6">
              <div className="flex justify-center items-center mb-3">
                {plan.name === "Champion" && <Crown className="w-6 h-6 text-yellow-400 mr-2" />}
                <h3 className="text-2xl font-bold">{plan.name}</h3>
              </div>
              <p className="text-gray-400">{plan.description || ""}</p>
              <div className="mt-4">
                <span className="text-5xl font-extrabold text-yellow-400">₹{plan.price}</span>
            ...
            <ul className="space-y-3 mb-8">
              {plan.features?.map((feature: string, i: number) => (
                <li key={i} className="flex items-start text-gray-300">
                  <Check className="w-4 h-4 text-yellow-400 mr-2 flex-shrink-0 mt-1" />
                  {feature}
                </li>
              ))}
            </ul>
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg font-semibold text-lg transition-all duration-300 flex items-center justify-center gap-2 ${processingPlanId === plan._id
                  ...
                  : plan.isActive
                    ? "bg-yellow-400 text-black shadow-md hover:shadow-yellow-400/50 hover:bg-yellow-500"
